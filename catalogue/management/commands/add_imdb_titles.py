...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "-i", "--imdb_ids", nargs="+", type=str, help="IMDb id (e.g. tt9698442"
        )
        ...
        for imdb_id in imdb_ids:
            try:
                ...
                if not exists:
                    self.stdout.write(
                        self.style.SUCCESS(
                            f'Title "{title.title}" was added successfully!'
                        )
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING(
                            f'Title "{title.title}" already exist in the database!'
                        )
                    )
            except TitleDoesNotExist:
                self.stdout.write(
                    self.style.ERROR(f'Title with imdb_id "{imdb_id}" does not exist.')
                )
